...
class Family:
    ...
    def reproduction(self):
        if const.PROBLEM == "TSP":
            indice = random.randint(0,const.SIZE_POPULATION)
        elif const.PROBLEM == "MAX_ONE":
            indice = random.randint(0,const.SIZE_BINARY)
        else:
            raise Exception("Aucun problème n'a été défini")

        city_list_children1_crossover = self.crossover(self.get_parent1().get_individu_list()[:indice],
                                                       self.get_parent2().get_individu_list(), indice)
        ...
        city_list_children2_crossover = self.crossover(self.get_parent2().get_individu_list()[:indice],
                                                       self.get_parent1().get_individu_list(), indice)
